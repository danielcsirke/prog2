...
struct Stack
{
	...
	/*
	Hungarian Notation : p - pointer, Data - adat(Simonyi Károly fia az excel fejlesztésében vett részt,
	tőle származik a változónév reprezentálja az általa adott típust ilyet is lehet : lpcwzstrText;
	háziban a pointernek p betűvel kell így kezdődnie másképp pontlevonás->referenciát r betűvel.
	*/
...
int stack_init(Stack& s, const Stack& other)
{
	...
	s.elements = other.elemets;

	// 2. Az s.pData beállítása az other.pData felhasználásával
	// s.pData = other.pData; // esélyes hogy akarjuk módosíítani a stacket, ekkor komoly problémákba ütközünk. u.a. mint s2 = s;

	s. pData = (char*)malloc(s.elements);
...
int stack_push(struct Stack& s, char newElement)
{
	...
	*/
	...
	if (!ptrTemp) // ugyan az mint if(ptrTemp == nullptr) vagy if(ptrTemp == NULL)
	{
		return -1;
	}

	// 3. Egy for ciklussal átmásoljuk az s.pData tartalmát a ptrTemp által mutatott
	// területre. (meg lehene csinlni memcpy függévnnyel is.)
...
int stack_pop(struct Stack& s)
{
	...
	a megállapodásnak megfeleloen állítjuk be az struktúra adattagjait.*/
